...
```{r table1md, echo=FALSE, message=FALSE, warning=FALSE}
# Mean and Std
...
table1 <- mk.df %>%
  ...
  table1  %>%  as_kable(format = "pipe")
```

```{r table1, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
sex.table  %>%  as_kable(format = "pipe")
```

```{r sex.table.md, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
sex.table 
```
...
table3 %>% as_kable(format = "pipe")
```


```{r table3md, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
table3 %>% 
    as_kable_extra(
      booktabs = TRUE,
      longtable = TRUE,
...
table.gta %>% as_kable(format = "pipe")
```

```{r gta.table.md, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=4, eval=FALSE}
table.gta %>% as_kable_extra(
    booktabs = TRUE,
    longtable = TRUE,
    linesep = ""
  ) %>%
...
table.sub %>% as_kable(format = "pipe")
```

```{r sub.stats.md, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
table.sub %>%
  as_kable_extra(
    booktabs = TRUE,
    longtable = TRUE,
...
p <- lavaanPlot(model = fit,
                coefs = TRUE,
                stand = TRUE,
                sig = 0.05)

savepath <- dirname(rstudioapi::getSourceEditorContext()$path)
lavaanPlot::save_png(p, file.path(savepath, "figures", "figure-sem_1a.png"), width = 900, height = 600)

knitr::include_graphics("figures/figure-sem_1a.png")

```

## SEM model for Episodic Memory

```{r sem_episodic, echo=FALSE, fig.height=6, fig.width=10, message=FALSE, warning=FALSE}
model <- '
  # Label paths for the mediation
  sc.neighbors ~ a * mk6240.sig
...
savepath <- dirname(rstudioapi::getSourceEditorContext()$path)
lavaanPlot::save_png(p, file.path(savepath, "figures", "figure-sem_1b.png"), width = 900, height = 600)

knitr::include_graphics("figures/figure-sem_1b.png")
...
savepath <- dirname(rstudioapi::getSourceEditorContext()$path)
lavaanPlot::save_png(p, file.path(savepath, "figures", "figure-sem_1c.png"), width = 900, height = 600)

knitr::include_graphics("figures/figure-sem_1c.png")
```
...
p <- lavaanPlot(model = fit.full,
           coefs = TRUE,       # show coefficients
           stand = TRUE,       # standardized coefficients
           sig = 0.05)         # only show significant paths

savepath <- dirname(rstudioapi::getSourceEditorContext()$path)
lavaanPlot::save_png(p, file.path(savepath, "figures", "figure-sem_2a.png"), width = 1200, height = 800)

knitr::include_graphics("figures/figure-sem_2a.png")
...
fit.full.hip <- sem(model, data = mk.ses1, se = "bootstrap", bootstrap = 1000)
summary(fit.full.hip, standardized = TRUE, fit.measures = TRUE)

p1 <- lavaanPlot(model = fit.full.hip,
           coefs = TRUE,       # show coefficients
           stand = TRUE,       # standardized coefficients
           sig = 0.05)         # only show significant paths

savepath <- dirname(rstudioapi::getSourceEditorContext()$path)
lavaanPlot::save_png(p1, file.path(savepath, "figures", "figure-sem_2b.png"), width = 1200, height = 800)

```


```{r sem_full+hipplot, fig.width=10, fig.height=3, warning=FALSE, echo=FALSE, message=FALSE}
knitr::include_graphics("figures/figure-sem_2b.png")
```
